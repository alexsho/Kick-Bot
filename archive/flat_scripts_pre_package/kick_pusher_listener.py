...
CONFIG_PATH = Path(os.getenv("KICK_BOT_CONFIG", "bot_config.json"))
...
async def run_forever() -> None:
    ...
    while True:
        try:
            ...
            print(f"Websocket closed: {exc}. Reconnecting in {backoff_seconds:.1f}s")
        except Exception as exc:
            print(f"Listener error: {exc}. Reconnecting in {backoff_seconds:.1f}s")
